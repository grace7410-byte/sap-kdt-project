...
#### 피드백 해결 (🦚추가 3): 계정번호 중복 체크 (`CheckDuplicate` 신규)
...
#### 피드백 해결 (🦚추가 4): 회사코드/통화/조정계정유형 존재 여부 검증 (`CheckExist` 신규)
...
#### 피드백 해결 (🦚추가 6): 신규 생성 시 거래종료(Loevm) 편집 불가 처리
...
#### 피드백 해결 (🦚추가 7): 자재평가/전기키/차대변/회계결정코드 존재 여부 검증 (`CheckExist` 신규)
...
#### 피드백 해결 (🦚추가 8): 중복 데이터 생성 방지 (`CheckDuplicate` 신규)
...
## 진행 예정 내역
8/27 오늘은 EINA/EINE 관련 [05] 구매정보레코드 FS 요구사항이 나가는 날이라, 해당 스펙 진행.
