...
| Behavior Definition | `ZR_B07_MARA` / `ZC_B07_MARA` | - | Managed, with draft. Field Control(통화 readonly는 미적용 확인) + Determination 완료, Validation 4종 중 CheckInit/CheckCreated만 구현됨 | [`ZR_B07_MARA.bdef.asbdef`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/01_material-mgmt/bdef/ZR_B07_MARA.bdef.asbdef) |
| Behavior Implementation | `ZBP_R_B07_MARA` | - | `SetInitialDefault`/`SetReadOnly` 구현 완료. `CheckMaterial`/`CheckSLoc`은 본문 비어있음 확인(2026-08-17), `get_instance_authorizations`도 비어있어 덤프 발생 | [`zbp_r_b07_mara.clas.abap`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/01_material-mgmt/bimp/zbp_r_b07_mara.clas.abap) |
