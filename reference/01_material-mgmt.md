...
| Behavior Definition | `ZR_B07_MARA` / `ZC_B07_MARA` | - | Managed, with draft. WAERS는 동적 features 제어. Validation 7종 중 CheckInit/CheckCreated/CheckSpart/CheckBklas/CheckPositive 구현됨. `ZI_B07_MARATEXT`(alias MaraText) 자식 Behavior는 CheckMaraTextExist validation으로 확정(2026-08-24) | [`ZR_B07_MARA.bdef.asbdef`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/01_material-mgmt/bdef/ZR_B07_MARA.bdef.asbdef), [`ZC_B07_MARA.bdef.asbdef`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/01_material-mgmt/bdef/ZC_B07_MARA.bdef.asbdef) |
| Behavior Implementation | `ZBP_R_B07_MARA` | - | `lhc_zr_b07_mara`(Root)와 `lhc_maratext`(Child, alias MaraText) 두 핸들러 클래스로 구성. `SetInitialDefault`/`SetMeinsDefault`/`SetReadOnly`(무한루프 수정)/`get_instance_features`(조건부)/`CheckInit`/`CheckCreated`/`CheckMaterial`(중복 체크)/`CheckSLoc`(플랜트-저장위치 정합성)/`CheckSpart`/`CheckBklas`(3000/3300/7900/7920 범위 + 자재타입 조합, 2026-08-25 보강)/`CheckPositive`(Stprs/Peinh 0 방지, 2026-08-25 신규)/`CheckMaraTextExist` 구현 완료. `get_instance_authorizations`는 여전히 비어있어 덤프 발생 | [`zbp_r_b07_mara.clas.abap`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/01_material-mgmt/bimp/zbp_r_b07_mara.clas.abap) |
