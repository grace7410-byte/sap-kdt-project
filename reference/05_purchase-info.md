...
| Behavior Definition | `ZR_B07_EINA`/`zi_b07_eine` · `ZC_B07_EINA`/`ZC_B07_EINE` | - | Managed, with draft. 헤더: UUID 변환(`SetVendorMaterialUuid`, 2026-09-02 신규)/자동채번(Infnr)/기본값(Esokz·Ekorg·Ekgrp)/필수값·중복·레코드유형 Validation. 아이템: 기본값(Waers·Prdat)/플랜트 존재·가격단위 양수 Validation + Waers 동적 readonly(`features:instance`). `draft determine action Prepare`(2026-09-09 3개 Validation 등록, 그 전까진 빈 블록으로 남아있어 Fiori Draft 화면 저장 전 실시간 검증이 동작하지 않았음). Projection은 `use draft;` + `use action Prepare` 형태로 재작성. **예외 처리·추가 로직은 여전히 WIP** | [`ZR_B07_EINA.bdef.asbdef`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/05_purchase-info/bdef/ZR_B07_EINA.bdef.asbdef), [`ZC_B07_EINA.bdef.asbdef`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/05_purchase-info/bdef/ZC_B07_EINA.bdef.asbdef) |
| Behavior Implementation | `ZBP_R_B07_EINA` | - | `lhc_eina`(헤더, 2026-09-09 `lhc_zr_b07_eina`에서 개명): `SetVendorMaterialUuid`(신규, 2026-09-09 LFA1/MARA 조회 성공 여부를 각각 별도 변수로 추적하도록 수정 — 기존엔 MARA 조회의 `sy-subrc`가 LFA1 조회 결과를 덮어쓰는 버그가 있었음)/`SetDefaults`/`SetInfnrNumber`(2026-09-02 연도+뒤6자리 가공 제거, 채번 결과 그대로 사용)/`CheckRequired`(2026-09-02 LifUuid/MatUuid 대신 Lifnr/Matnr 검사로 수정 — 진짜 원인이었음)/`CheckDuplicate`/`CheckEsokz`. `lhc_eine`(아이템, alias `Eine`): `SetItemDefaults`/`CheckExist`/`CheckPositive`/`get_instance_features`(Waers). 미완성 WIP 상태 | [`zbp_r_b07_eina.clas.abap`](https://github.com/grace7410-byte/sap-kdt-project/blob/main/src/05_purchase-info/bimp/zbp_r_b07_eina.clas.abap) |
...
- `SetVendorMaterialUuid`의 `sy-subrc` 공유 버그를 발견/수정함 — LFA1 조회 후 곧바로 MARA 조회가 실행되면서 `sy-subrc`가 MARA 조회 결과로 덮어써지고, 그 값을 그대로 LifUuid COND에서 참조하고 있었음(즉 LifUuid 채움 여부가 실제로는 MARA 조회 성공/실패에 좌우되고 있었음). LFA1/MARA 조회 성공 여부를 `lv_lfa1_found`/`lv_mara_found`로 각각 분리 추적하도록 수정.
- 다만 아래 "존재하지 않는 Lifnr/Matnr 값에 대한 에러 처리 없음" 이슈 자체는 이번에도 반영되지 않음 — sy-subrc 버그만 고쳤을 뿐, 존재하지 않는 값 입력 시 별도 에러 메시지를 내는 로직은 여전히 미착수.
- 헤더 `draft determine action Prepare`가 빈 블록으로 남아있던 것을 발견 — `CheckRequired`/`CheckDuplicate`/`CheckEsokz` 3종을 등록해 06번 EKKO bdef와 동일한 패턴으로 맞춤(Fiori Draft 화면에서 저장 전 실시간 검증이 동작하도록).
- 헤더 핸들러 클래스명을 `lhc_zr_b07_eina` → `lhc_eina`로 변경(아이템 `lhc_eine`과 이름 패턴을 맞춤, 기능 영향 없음).
