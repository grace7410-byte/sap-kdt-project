...
- Waers는 정적 `field(readonly)`가 불가능해 `field(features:instance)` + `get_instance_features`로 전환(자세한 내용은 [8/31 개발일지](../devlog/rap-dev/2026-08-31.md) 참고).
- Projection BDEF(`ZC_B07_EINA`)에서 Create 버튼이 안 뜨는 문제를 겪어 파일을 삭제 후 재생성 — `use draft;` + `use action Prepare` 형태로 최종 정리.
- 화면 테스트: 기본값(기본단위/자재/공급업체)이 채워지는 것은 확인했으나, 나머지 Validation 로직은 아직 하나씩 확인 못함 — 다음 작업일에 이어서 진행.
